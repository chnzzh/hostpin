...
RUN CGO_ENABLED=0 GOOS="$TARGETOS" GOARCH="$TARGETARCH" go build -trimpath -ldflags="-s -w -X github.com/chnzzh/hostpin/internal/buildinfo.Version=${VERSION} -X github.com/chnzzh/hostpin/internal/buildinfo.Commit=${COMMIT} -X github.com/chnzzh/hostpin/internal/buildinfo.ReleaseBase=${RELEASE_BASE}" -o /out/hostpin-server ./cmd/hostpin-server
...
HEALTHCHECK --interval=15s --timeout=5s --start-period=10s --retries=5 CMD wget -q -O - http://127.0.0.1:8080/readyz || exit 1
STOPSIGNAL SIGTERM
