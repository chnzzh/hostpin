...
SQLite is the default database in every example below.

### Linux host (one command)
...
restricted `hostpin` service account, writes the SQLite configuration, and
starts `hostpin-server`. Data is stored in `/var/lib/hostpin`, while the
...
For development or unsupported server platforms, build with Go 1.26.x and the
locked pnpm version:
...
This local path creates `./data/hostpin.db`. Open
`http://localhost:8080/setup`, create the administrator and enrollment PIN,
then install an Agent from the command shown in the dashboard.
...
Production enrollment should use HTTPS. See [`docs/deployment.md`](docs/deployment.md)
for installer options, manual binary installation, reverse proxy, PostgreSQL,
