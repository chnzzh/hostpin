...
| Server one-command install, URL/default-listener behavior, checksum, SQLite config, unsafe HTTP rejection, and upgrade preservation | `tests/e2e/server_installer.sh` in CI and `make release-check` |
...
the staged server-installer test, and scheduled capacity results to pass.
Server logs are scanned for panic, fatal, and error-level failures.
