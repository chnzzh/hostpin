...
| 服务端一键安装、URL 问答、默认监听、校验、SQLite 配置、公网 HTTP 显式授权与升级保留 | CI 和 `make release-check` 中的 `tests/e2e/server_installer.sh` |
| Agent 私网/公网 HTTP 确认和自动化开关 | `internal/installer/install_test.go`、浏览器与 Linux 安装流程 |
| Dockerfile、直接拉取 Compose、非 root 启动和 `/readyz` | CI `container` 作业与发布前镜像冒烟测试 |
