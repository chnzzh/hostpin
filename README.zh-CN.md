...
SQLite 配置并启动 systemd 服务。数据位于 `/var/lib/hostpin`，配置位于
...
开发环境或服务端平台不受一键脚本支持时，可使用 Go 1.26.x 和仓库锁定的 pnpm
版本构建：
...
此方式会自动创建 `./data/hostpin.db`。打开 `http://localhost:8080/setup`，创建
管理员账户和注册 PIN，然后执行面板显示的 Agent 安装命令：
